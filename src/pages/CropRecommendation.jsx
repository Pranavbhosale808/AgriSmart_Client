  ...
  const CropRecommendation = () => {
    ...
    const onSubmit = async (formData) => {
      ...
      try {
        // Modify formData before sending it
        const modifiedData = {
          N: formData.n, // Ensure proper key names
          P: formData.p,
          K: formData.k,
          temperature: parseFloat(formData.temperature).toFixed(2), // Format temperature
          humidity: parseFloat(formData.humidity).toFixed(2), // Format humidity
          ph: parseFloat(formData.ph).toFixed(2), // Format pH
          rainfall: parseFloat(formData.rainfall).toFixed(2) // Format rainfall
        };
    
        console.log("Modified Data:", modifiedData);
    
        // Convert modified data to query params
        const queryParams = new URLSearchParams(modifiedData).toString();
        
        const response = await fetch(`http://127.0.0.1:8000/api/crop?${queryParams}`);
...
{result && (
  ...
  >
    ...
    {/* Mobile View: Stacked Data */}
    <div className="block md:hidden border border-gray-300 rounded-lg p-4">
      <div className="flex flex-col items-center text-center space-y-4">
        <img src={result.image} alt={result.crop} className="w-24 h-24 object-cover rounded-md shadow-md" />
        <div className="font-bold text-green-700 text-xl">{result.crop} <span className="text-gray-500">({result.translation})</span></div>
        <div><span className="font-semibold">Soil Type:</span> {result.soil_type}</div>
        <div><span className="font-semibold">Best Growing Conditions:</span> {result.conditions}</div>
      </div>
    </div>

    {/* Desktop View: Normal Table */}
    <table className="hidden md:table w-full border-collapse border border-gray-300 text-gray-700">
      <thead>
